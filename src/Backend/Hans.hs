module Backend.Hans(initializeBackend)
 where
  ...
  do con <- initXenConsole
     handle (handleErr con "at top level") $
       do xs   <- initXenStore
          ...
          leases <- mapM getLease devs
          writeConsole con ("Generated " ++ show (length leases) ++
                            " potential leases.\n")
          processAsyncs startDevice 
...
getLease :: Device -> 
          ...
          processAsyncs leases $ \ (mlease, dev) ->
            case mlease of
              Nothing ->
                writeConsole con ("Device " ++ show (devMac dev) ++ " failed to get an  address.\n")
              Just lease ->
     return Backend {
       listen = undefined
     , accept = undefined
